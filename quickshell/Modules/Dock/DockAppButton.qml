...
Item {
    ...
    // Untransformed button center in dockMouseArea coords (avoid the
    // magnification feedback loop — measure layout position, not the
    // visually-shifted one).
    ...
    readonly property real _dist: {
        if (!_magEnabled)
            return 9999;
        const d = _cursorPos - _center;
        return d; // signed; magnitude checked against radius
    }
    ...
    // Each icon is nudged away from the cursor proportional to its scale,
    // matching the reference's `x = (-d/DIST) * NUDGE * scale` (no neighbor
    // propagation — the per-icon scale already produces the spread).
